...
class plot3D(gl.GLViewWidget):  
    ...
    def save_plot(self):
        ...
        if len(directory)>4:
            image=self.get_image_to_save()
            image.save(file_directory)
            self.save_colorbar(directory)
            
    def export_plot(self,folder_path):
        print('get image to save')
        image=self.get_image_to_save()
        print('try to save')
        image.save(folder_path+'/temp3d.png')
        print('try to read and send')
        return cv2.imread(folder_path+'/temp3d.png')
