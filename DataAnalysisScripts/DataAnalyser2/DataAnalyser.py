...
class CentralWidget(QtWidgets.QWidget):
    ...
    def record_change(self,isRecording):
        ...
        if isRecording:
            ...
            options_recording.recorded_folder_path.connect(self.create_video)
            options_recording.exec_()
        else:
            self.terminate_video()
            
    def create_video(self,recorded_folder_path):
    ...
    def add_frame(self,img): #img is the trigger for adding a new image

        plotx=self.xplot.export_plot(self.recorded_folder_path)
        ploty=self.yplot.export_plot(self.recorded_folder_path)
        plotz=self.zplot.export_plot(self.recorded_folder_path)
        plot3d=self.plot3D.export_plot(self.recorded_folder_path)

        if not(self.initiated):
            size1 = img.shape[1], img.shape[0]
            self.vid_image=VideoWriter(self.recorded_folder_path+"/images.avi", self.fourcc, float(self.fps), size1, self.is_color)
            size2 = plotx.shape[1], plotx.shape[0]
            self.vid_plotx=VideoWriter(self.recorded_folder_path+'/plotx.avi', self.fourcc, float(self.fps), size2, self.is_color)
            size3 = ploty.shape[1], ploty.shape[0]
            self.vid_ploty=VideoWriter(self.recorded_folder_path+'/ploty.avi', self.fourcc, float(self.fps), size3, self.is_color)
            size4 = plotz.shape[1], plotz.shape[0]
            self.vid_plotz=VideoWriter(self.recorded_folder_path+'/plotz.avi', self.fourcc, float(self.fps), size4, self.is_color)
            size5 = plot3d.shape[1], plot3d.shape[0]
            self.vid_plot3d=VideoWriter(self.recorded_folder_path+'/plot3d.avi', self.fourcc, float(self.fps), size5, self.is_color)
            self.initiated=True
...
class options_Recording(QtGui.QDialog):
    recorded_folder_path=QtCore.pyqtSignal(str)
    ...
    def __init__(self,parent=None):
        ...
        self.folder_path='C:/Users/Francois/Desktop'
    ...
    def pick_new_directory(self):
        ...
        self.recorded_folder_path.emit(self.folder_path)
