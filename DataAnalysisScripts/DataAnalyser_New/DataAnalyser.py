...
#            ymax+=delta_y
        ...
        if xmax-xmin > 15 or (xmin<-7.5 or xmax> 7.5):
            delta_x= -np.mean(self.Xobjet)
        ...
        if (ymax-ymin > 3 or (ymin<0 or ymax > 3)):
            delta_y= -(np.mean(self.Yobjet) - 1.5)
            self.Yobjet += delta_y
            ymin = self.Yobjet.min()
            ymax = self.Yobjet.max()
...
class plot3D(gl.GLViewWidget):  
    ...
    def __init__(self, parent=None):
        ...
        self.size_traj=0.12
        self.size_grid=1.
        self.size_marker= 1.0           # Modified to make marker larger
